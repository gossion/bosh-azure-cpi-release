...
module Bosh::AzureCloud
  ...
  class AzureClient2
    ...
    def list_load_balancers(resource_group_name)
      load_balancers = []
      url = rest_api_url(REST_API_PROVIDER_NETWORK, REST_API_LOAD_BALANCERS, resource_group_name: resource_group_name)
      result = get_resource_by_id(url)
      unless result.nil? || result['value'].nil?
        result['value'].each do |ret|
    ...
    def parse_network_interface(result, recursive: true)
      ...
      unless result.nil?
        ...
        unless ip_configuration_properties['loadBalancerBackendAddressPools'].nil?
          load_balancers = []
          ip_configuration_properties['loadBalancerBackendAddressPools'].each do |lb|
            if recursive
              names = parse_name_from_id(lb['id'])
              load_balancers.push(get_load_balancer_by_name(names[:resource_name]))
            else
              load_balancers.push({:id => lb['id']})
            end
          end
