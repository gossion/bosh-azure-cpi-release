...
module Bosh::AzureCloud
  class VMManager
    ...
    def initialize(azure_config, registry_endpoint, disk_manager, disk_manager2, azure_client2, storage_account_manager, stemcell_manager, stemcell_manager2, light_stemcell_manager)
    ...
    def create(instance_id, location, stemcell_id, vm_properties, network_configurator, env)
      ...
      task_create_network_interfaces = Concurrent::Future.execute do
        create_network_interfaces(resource_group_name, vm_name, location, vm_properties, network_configurator, primary_nic_tags)
      end
    ...
    def create_network_interfaces(resource_group_name, vm_name, location, vm_properties, network_configurator, primary_nic_tags = AZURE_TAGS)
      ...
      task_get_or_create_public_ip = Concurrent::Future.execute do
        get_or_create_public_ip(resource_group_name, vm_name, location, vm_properties, network_configurator)
      end
      task_get_load_balancer = Concurrent::Future.execute do
        get_load_balancer(vm_properties)
      end
      task_get_application_gateway = Concurrent::Future.execute do
        get_application_gateway(vm_properties)
      end
